...
app.post('/process', async (req, res) => {
  try {
    ...
    // **Fetch Customer Info from MainBackend**
    let customerInfo = null;
    try {
      // Assuming MainBackend is on localhost:3000
      const fetch = (await import('node-fetch')).default;
      const customerResp = await fetch('http://localhost:3000/active-call');
      if (customerResp.ok) {
        const customerData = await customerResp.json();
        if (customerData.active && customerData.call && customerData.call.customer) {
          customerInfo = customerData.call.customer;
          console.log(`[Report Agent] Fetched customer info: ${customerInfo['이름']}`);
        } else {
          console.log('[Report Agent] No active call/customer info found.');
        }
      }
    } catch (err) {
      console.warn('[Report Agent] Failed to fetch customer info:', err.message);
    }
    ...
    // Pass customerInfo to generateReport
    const reportContent = await generateReport(analysis, 'markdown', customerInfo);
    const reportData = createReportMetadata(analysis, reportContent);
