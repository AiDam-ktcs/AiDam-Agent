...
★ 핵심 지침: 고객의 의중(Intent)을 최우선으로 고려하세요.
- 의중이 '가격 민감(price_sensitive)'이라면: "혜택"보다는 "가성비"나 "절약"을 강조하세요. 비싼 요금제를 추천해야 한다면 "커피 한 잔 값으로 얻는 이득" 등으로 설득하세요.
- 의중이 '데이터 부족(data_hungry)'이라면: "넉넉한 데이터"와 "속도 제한 없는 쾌적함"을 강조하세요.
- 의중이 '불만(complaint)'이라면: 먼저 불편함에 깊이 공감하고, 이 요금제가 그 해결책임을 제시하세요.

맥락 정보:
- 고객 의중: {customer_intent} ({intent_description})
...
작성 스타일:
- 구어체로 자연스럽게 (딱딱하지 않게)
- 고객의 상황을 언급하며 공감대 형성 ("아까 데이터 때문에 답답하다고 하셨는데...")
- 3~4문장 내외로 간결하지만 임팩트 있게
- "고객님,"으로 시작
"""),
            ("human", """대화 이력:
{conversation_history}

위 대화 흐름을 고려하여, 발견된 의중('{customer_intent}')에 딱 맞는 맞춤형 제안 멘트를 작성해주세요.""")
        ])
